...
for i=2:10
    ...
    if i==8
        Connect3D(Link(7).p,Link(i).p,'r',2); hold on;
    end
    if i==9
        Connect3D(Link(7).p,Link(i).p,'b',2); hold on;
    end
    if i==10
        Connect3D(Link(7).p,Link(i).p,'g',2); hold on;
    end
...
grid on;
view(134,12);
axis([-2000,2000,-2000,2000,-2000,3000]);
xlabel('x');
ylabel('y');
zlabel('z');
drawnow;
